 ...
 * 
 */
public class MendixObjToString extends CustomJavaAction<String>
{
	...
	public MendixObjToString(IContext context, IMendixObject ObjectToPrint, String OutputTypeValue, String DateFormat, Boolean WithAssociations)
	{
		super(context);
	...
	private static String objectToString(IContext c,
			IMendixObject objectToPrint, Boolean withAssociations,
			String dateFormat, logging.proxies.OutputType outputType,
			boolean rootCall) {
		StringBuilder sb = new StringBuilder();
		StringBuilder endTags = new StringBuilder();
		Map<String, ? extends IMendixObjectMember<?>> members = objectToPrint
				.getMembers(c);
		...
		for (String key : sorted) {
			...
			if (m.getValue(c) == null) {
				...
				if (rootCall) {
					...
					com.mendix.core.objectmanagement.MendixIdentifier referencedObjectID = objectToPrint
							.getValue(c, mr.getName());
					try {
						IMendixObject referencedObject = Core.retrieveId(c,
								referencedObjectID);
						sb.append(objectToString(c, referencedObject,
								withAssociations, dateFormat, outputType, false));
	...
	private static void addFields(String columnName, String value,
			StringBuilder sb, logging.proxies.OutputType outputType,
			JSONObject jsonObj) {
	...
	private static void startTag(String columnName, StringBuilder sb,
			logging.proxies.OutputType outputType, StringBuilder endTags,
			JSONObject jsonObj) {
		if (outputType.getCaption().equals("String")) {
			sb.append(" ");
			sb.append(columnName);
